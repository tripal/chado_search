...
function chado_search_create_sequence_search_mview() {
  ...
  $sql = "SELECT DISTINCT
      ...
      WHERE AP.value IN('reftrans', 'unigene', 'whole_genome', 'tripal_analysis_unigene', 'bulk_data', 'ncbi_data', 'transcriptome', 'other_transcripts') OR AP.value IS NULL
      ...
      WHERE F.type_id IN (SELECT cvterm_id FROM cvterm WHERE name IN ('gene', 'mRNA', 'contig', 'EST') AND cv_id = (SELECT cv_id FROM cv WHERE name = 'sequence'))";
