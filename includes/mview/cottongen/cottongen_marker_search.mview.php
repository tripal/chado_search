...
function chado_search_create_marker_search_mview() {
  ...
  $sql = "SELECT DISTINCT
      ...
      LEFT JOIN (SELECT featuremap_id, value FROM featuremapprop FMP INNER JOIN cvterm ON type_id = cvterm_id WHERE cvterm.name = 'genome_group') MAPGENOME ON MAPGENOME.featuremap_id = MAP.featuremap_id 
      LEFT JOIN (SELECT feature_id, value FROM featureprop FP WHERE FP.type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'chr_name')) Chr_name ON Chr_name.feature_id = MAP.map_feature_id 
