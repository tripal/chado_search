...
function chado_search_create_snp_genotype_search_mview() {
  ...
  --- Select Allele
      (SELECT value FROM featureprop WHERE feature_id = F.feature_id AND type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'allele' AND cv_id = (SELECT cv_id FROM cv WHERE name = 'sequence'))) AS allele,
  --- Select genotype
