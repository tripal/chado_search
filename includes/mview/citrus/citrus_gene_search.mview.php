...
-- Restrict the sequence type to gene/mRNA/contig for Unigene/RefTrans
      ...
      (A.analysis_id IN (SELECT analysis_id FROM analysisprop WHERE type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'Analysis Type') AND value IN ('reftrans', 'unigene', 'tripal_analysis_unigene', 'bulk_data', 'ncbi_data', 'citrus_gene_database', 'transcriptome', 'other_transcripts')))
      OR
