...
function chado_search_create_transcript_search_mview() {
  $view_name = 'chado_search_transcript_search';
...
(SELECT string_agg(distinct 
  ...
  INNER JOIN analysisfeature AF2 ON AF2.analysisfeature_id = AFP.analysisfeature_id
  WHERE 
    type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'kegg_brite_data')
  AND
  AF2.feature_id = F.feature_id
) AS kegg_value,
-- Interpro
...
SELECT analysis_id FROM analysisprop WHERE type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'analysis_unigene_name'))
OR A.analysis_id IN (SELECT analysis_id FROM analysis WHERE program = 'reftrans'))
-- Restrict the sequence type to contig and EST singlet
AND (F.type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'contig' AND cv_id = (SELECT cv_id FROM cv WHERE name = 'sequence')) 
OR (F.type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'EST' AND cv_id = (SELECT cv_id FROM cv WHERE name = 'sequence'))
AND (SELECT first(value) FROM featureprop FP WHERE FP.feature_id = F.feature_id AND type_id = (SELECT cvterm_id FROM cvterm WHERE name = 'singlet_in_analysis')) IS NOT NULL
)
)
  ";
